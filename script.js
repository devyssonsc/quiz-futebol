...
$divAnswers.addEventListener("click", function(event) {
    if (event.target.tagName === "BUTTON") {
        ...
        console.log(event.target.innerText.toLowerCase() === questionsCopy[sortedQuestion].correctAnswer.toLowerCase());
        if (event.target.innerText.toLowerCase() === questionsCopy[sortedQuestion].correctAnswer.toLowerCase()) {
        ...
        if (questionsCopy.length > 0) {
            // Se houver, continue com a próxima pergunta
            sortQuestion();
        } else {
            // Caso contrário, o quiz terminou
            alert(`Quiz concluído! Pontuação total: ${totalPoints} pontos`);
        }
    }
});
